    ...
    <div className='flex flex-col m-4 gap-4 max-w-[1000px] self-center'>
      {departments.map((dept, index) => (
        <div key={index} className='w-[100%] h-fit relative border-2 rounded-lg border-b-4 border-b-Dblue flex flex-col'>
          ...
          <img src={dept.image || `/department${index + 1}.png`} alt="" className='w-auto sm:h-64 h-[100%] m-2 object-contain sm:absolute sm:right-8 sm:top-1/2 sm:transform sm:-translate-y-1/2 sm:rounded-full' />
          <div className='sm:w-[65%] w-[100%] h-fit text-gray-700 p-4 mb-8 gap-2 flex flex-col font-medium'>
            <li>{dept.description}</li>
          </div>
        </div>
      ))}

      {/* Add Department Form */}
      <div className='w-[100%] h-fit relative border-2 rounded-lg border-b-4 border-b-Dblue flex flex-col p-4'>
        ...
        <input
          ...
          className='p-2 border rounded-md mb-2 w-full'
        />
        <button onClick={handleAddDepartment} className='bg-Dblue text-white p-2 rounded-md'>Add</button>
      </div>
      ...
      <div className='w-[100%] h-fit relative border-2 rounded-lg border-b-4 border-b-Dblue flex flex-col p-4'>
        ...
        <input
          ...
          className='p-2 border rounded-md mb-2 w-full'
        />
        <button onClick={handleDeleteDepartment} className='bg-red-600 text-white p-2 rounded-md'>Delete</button>
      </div>
    </div>
  );
};
